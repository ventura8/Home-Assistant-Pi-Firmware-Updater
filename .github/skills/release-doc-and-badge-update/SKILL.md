...
7. **Cut the tag (local, then confirm before publishing)**
   ...
   - **Do not push the tag without the user's explicit go-ahead** — a pushed
     tag triggers `.github/workflows/release.yml`, which publishes the GitHub
     Release from `docs/releases/vX.Y.Z_github_description.md` (H1 → title,
     file body → notes). Report the tag is staged locally and ask before
     pushing.
   - Once confirmed, push only the tag (CI creates the Release — do **not**
     also run `gh release create` unless backfilling a tag that predates the
     workflow):
   ...
   - To publish an **existing** tag that never got a Release (or to re-run
     after fixing notes on that tag), use Actions → **Publish GitHub Release**
     → Run workflow with the tag input, or:
...
- [ ] Tag push only after explicit user confirmation; GitHub Release is created
      by `.github/workflows/release.yml` from `vX.Y.Z_github_description.md`
- [ ] Existing remote tags were not force-moved; local recreate only when remote
      tag is absent
- [ ] Do not duplicate with a manual `gh release create` when CI will publish
...
- Ask before pushing a tag (CI publishes the GitHub Release)
- Prefer `gh workflow run release.yml -f tag=vX.Y.Z` to backfill an existing tag
...
- Manually `gh release create` for tags that the release workflow will publish
  (unless CI is unavailable and the user asked for a manual backfill)
