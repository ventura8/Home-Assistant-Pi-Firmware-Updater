...
6. **Stop and ask before publishing** — pushing the tag is visible to others and
   triggers `.github/workflows/release.yml` to create the GitHub Release from
   `docs/releases/vX.Y.Z_github_description.md`. Only push after the user
   confirms. Do not also run `gh release create` unless backfilling.
...
- Tag name is `v` + `manifest.json` `"version"`; tag only pushed after
  confirmation; Release published by CI (or workflow_dispatch backfill)
